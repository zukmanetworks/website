...
namespace Elementor;
class Appside_Testimonial_One_Widget extends Widget_Base {
	...
	public function get_name() {
		return 'appside-testimonial-one-widget';
	}
	...
	public function get_title() {
		return esc_html__( 'Testimonial: 01', 'aapside-master' );
	}
	...
	protected function _register_controls() {
		...
		$this->add_control( 'testimonial_items', [
			...
			'default' => [
				[
                    ...
                    'description' => esc_html__('They  provide innovative solutions with the best.  tempor incididunt utla bore et dolor  tempor incididunt .','aapside-master')
				],[
					'name'        => esc_html__( 'Riley Cassidy', 'aapside-master' ),
					'image'       => array(
						'url' => Utils::get_placeholder_image_src()
		...
		$this->add_control('name_color',[
			...
			'selectors' => [
				"{{WRAPPER}} .single-testimonial-item .hover .hover-inner .author-meta .name" => "color: {{VALUE}}"
			]
		]);
		$this->add_control('designation_color',[
			...
			'type' => Controls_Manager::COLOR,
			'selectors' => [
				"{{WRAPPER}} .single-testimonial-item .hover .hover-inner .author-meta .post" => "color: {{VALUE}}"
			]
		]);
		$this->add_control('description_color',[
			...
			'type' => Controls_Manager::COLOR,
			'selectors' => [
				"{{WRAPPER}} .single-testimonial-item .hover .hover-inner p" => "color: {{VALUE}}"
			]
		]);
		$this->add_control('ratings_color',[
			'label' => esc_html__('Ratings Color','aapside-master'),
			'type' => Controls_Manager::COLOR,
			'selectors' => [
				"{{WRAPPER}} .single-testimonial-item .hover .hover-inner .author-meta .ratings" => "color: {{VALUE}}"
			]
		]);
        ...
        $this->add_control('icon_color',[
	        ...
	        'type' => Controls_Manager::COLOR,
	        'selectors' => [
	        	"{{WRAPPER}} .single-testimonial-item .hover .hover-inner .icon" => "color: {{VALUE}}"
	        ]
        ]);
		$this->add_group_control(Group_Control_Background::get_type(),[
			'name' => 'testimonial_hover_background',
			'selector' => "{{WRAPPER}} .single-testimonial-item .hover .hover-inner .icon",
		...
		$this->add_group_control(Group_Control_Typography::get_type(),[
			...
			'selector' => "{{WRAPPER}} .single-testimonial-item .hover .hover-inner .author-meta .name"
		]);
		$this->add_group_control(Group_Control_Typography::get_type(),[
			'name' => 'designation_typography',
			'label' => esc_html__('Designation Typography','aapside-master'),
			'selector' => "{{WRAPPER}} .single-testimonial-item .hover .hover-inner .author-meta .post"
		]);
		$this->add_group_control(Group_Control_Typography::get_type(),[
			'name' => 'description_typography',
			'label' => esc_html__('Description Typography','aapside-master'),
			'selector' => "{{WRAPPER}} .single-testimonial-item .hover .hover-inner p"
		]);
		$this->end_controls_section();
...
Plugin::instance()->widgets_manager->register_widget_type( new Appside_Testimonial_One_Widget() );